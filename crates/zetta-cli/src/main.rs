...
fn parse_tui_slash_command(input: &str) -> Option<Result<ReplCommand, String>> {
    parse_local_command(input, '/')
}
...
fn pane_title(base: &str, scroll: usize, unseen: usize) -> String {
...
fn render_markdown_styled_lines(input: &str, base_style: Style) -> Vec<StyledTextLine> {
...
fn clamp_cursor_boundary(input: &str, cursor: usize) -> usize {
    if cursor >= input.len() {
        return input.len();
    }
...
fn session_overview_text(session: &zetta_protocol::SessionSnapshot) -> String {
